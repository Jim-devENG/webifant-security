...
const ClientLogin = () => {
    ...
    const handleSubmit = async ( e ) => {
        ...
        try {
            ...
            // Set basic authentication first
            localStorage.setItem( 'clientAuthenticated', 'true' );
            localStorage.setItem( 'clientId', userCredential.user.uid );
            localStorage.setItem( 'clientEmail', userCredential.user.email );

            // Set basic user info without profile check
            localStorage.setItem( 'clientName', userCredential.user.displayName || 'Unknown' );
            console.log( 'Basic authentication set successfully' );

            console.log( 'Client logged in successfully:', userCredential.user.uid );
            console.log( 'localStorage clientAuthenticated:', localStorage.getItem( 'clientAuthenticated' ) );
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            ...
            // Use Firebase's built-in Google sign-in directly
            ...
            // Set authentication immediately without Firestore queries
            localStorage.setItem( 'clientAuthenticated', 'true' );
            localStorage.setItem( 'clientId', user.uid );
            localStorage.setItem( 'clientEmail', user.email );
            localStorage.setItem( 'clientName', user.displayName || 'Unknown' );

            console.log( 'Client logged in with Google successfully:', user.uid );
            console.log( 'localStorage clientAuthenticated:', localStorage.getItem( 'clientAuthenticated' ) );
            ...
            <div className="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 pt-24">
                ...
                <div className="relative z-10 max-w-md w-full">
                    ...
                    <div className="text-center mb-8">
                        <div className="mb-6">
                            <div className="inline-flex items-center justify-center w-16 h-16 bg-cyan-400/20 rounded-full border border-cyan-400/30 mb-4">
                            ...
                            <p className="text-gray-300">
                                Sign in to your WebInfant account
                            </p>
                        </div>
                    </div>

                    {/* Login Form */}
                    <div className="bg-gray-900/50 backdrop-blur-lg border border-cyan-400/20 rounded-2xl p-8 shadow-2xl">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 bg-gray-800/50 border border-cyan-400/30 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400 transition-all duration-300"
                        ...
                        <div className="mt-8 text-center space-y-4">
                            <p className="text-gray-400">
                                ...
                                <a href="/client-registration" className="text-cyan-400 hover:text-cyan-300 font-medium transition-colors">
                                    Register here
                                </a>
                            </p>
                            <p className="text-gray-400">
